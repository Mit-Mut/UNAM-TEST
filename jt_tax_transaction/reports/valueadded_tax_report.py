...
#                     
#                     new_col.append(self._format({'name': float_amount},figure_type='float'))
#                 else:
#                     new_col.append(data)
#                               
#             line.update({'columns':new_col})
        ...
        if col_list:
            ...
            new_lines.append({'id': 'tax_charge_favor', 'name': _('Tax in Charge / Favor'), 
                           'unfoldable': False, 
                           'columns': columns,
                           'level': 1, 
                           }           
    ...
    def get_pdf(self, options, minimal_layout=True,line_id=None):
        ...
        if minimal_layout:
            ...
            header = self.env['ir.actions.report'].render_template("jt_tax_transaction.external_layout_value_added_tax", values=rcontext)
...
CONTADURÍA GENERAL\nREPORTE PARA LA DETERMINACIÓN DEL IMPUESTO AL\nVALOR AGREGADO A CARGO %s
''' % (header_date)
