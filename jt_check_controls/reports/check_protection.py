...
class CheckProtection(models.AbstractModel):

    _name = "jt_check_controls.check_protection"
    _inherit = "account.coa.report"
    _description = "Check Protection"
    ...
    def _get_templates(self):
        templates = super(
            CheckProtection, self)._get_templates()
        templates[
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if deps:
            ...
            options.update({'total_check': total_check, 'total_amount': total_amount})                 
        return lines
...
#                 total_amt += str(p.get('no_format_name'))
            total_check = options.get('total_check',0)
            total_amt = options.get('total_amount',0)
            ...
            rcontext.update({
                    ...
                    'total_amt':str(self._format({'name': total_amt},figure_type='float').get('name')),
                    'date_list':date_list
...
\nREPORTE DIARIO DE PROTECCIÓN DE CHEQUES
'''
