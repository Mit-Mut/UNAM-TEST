...
class StatePartimonialSituation(models.AbstractModel):
    _name = "jt_account_design.state.partimonial.situation"
    _inherit = "account.coa.report"
    _description = "State Partimonial Situation"
    ...
    def _get_templates(self):
        templates = super(
            StatePartimonialSituation, self)._get_templates()
        templates[
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for con in concept_ids:
            ...
            for acc in account_ids:
                balance = 0
                values= self.env['account.move.line'].search([('date', '<=', end),('account_id', '=', acc.id),move_state_domain])
    ...
    def _get_report_name(self):
        return _("State Partimonial Situation")
