...
class ComparisonOfBalanceCheck(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        project_domain += [('proj_start_date', '>=', start),
                           ('proj_end_date', '<=', end), ('project_type', '!=', 'other')]
        ...
        if stage_ids:
            ...
            lines.append({
                ...
                'name': 'ETAPA/ANO',
                'columns': [
                        {'name': 'POR COMPROBAR A DIC/' +
                            str(two_year_ago.year)},
                        {'name': 'POR COMPROBAR A DIC/' +
                            str(one_year_ago.year)},
                        {'name': 'POR COMPROBAR A ' +
                            self.get_month_name(end.month) + "/" + str(end.year)},
                        {'name': 'COMPROBADO EN EL PERIODO ' + self.get_month_name(
                            first_month_date.month) + "/" + self.get_month_name(end.month)},
                ],
                'level': 1,
                'unfoldable': False,
                'unfolded': True,
        ...
        for stage in stage_ids:
            for year in year_list:
                ...
                if not current_project_ids:
                    continue
                cfdi_account_ids = current_project_ids.mapped(
                ...
                if cfdi_account_ids:
                    values = self.env['account.move.line'].search([('date', '<=', two_year_ago), (
                        'account_id', 'in', cfdi_account_ids.ids), ('move_id.state', '=', 'posted')])
                    two_year_ago_amount = sum(
                        x.debit - x.credit for x in values)

                if cfdi_account_ids:
                    values = self.env['account.move.line'].search([('date', '<=', one_year_ago), (
                        'account_id', 'in', cfdi_account_ids.ids), ('move_id.state', '=', 'posted')])
                    one_year_ago_amount = sum(
                        x.debit - x.credit for x in values)

                if cfdi_account_ids:
                    values = self.env['account.move.line'].search(
                        [('date', '<=', end), ('account_id', 'in', cfdi_account_ids.ids), ('move_id.state', '=', 'posted')])
                    current_year_amount = sum(
                        x.debit - x.credit for x in values)
                ...
                lines.append({
                    ...
                    'columns': [
                        self._format({'name': two_year_ago_amount},
                                     figure_type='float'),
                        self._format({'name': one_year_ago_amount},
                                     figure_type='float'),
                        self._format({'name': current_year_amount},
                                     figure_type='float'),
        ...
        if stage_ids:
            lines.append({
                ...
                'columns': [
                        self._format(
                            {'name': total_two_year_ago_amount}, figure_type='float'),
                        self._format(
                            {'name': total_one_year_ago_amount}, figure_type='float'),
                        self._format(
                            {'name': total_current_year_amount}, figure_type='float'),
                        self._format({'name': total_different},
                                     figure_type='float'),
                ],
                'level': 1,
                'unfoldable': False,
        ...
        project_domain += [('proj_start_date', '>=', start), ('proj_end_date',
                                                              '<=', end), ('project_type', 'not in', ('conacyt', 'concurrent'))]
        ...
        lines.append({
            ...
            'columns': [
                    ...
                    self._format({'name': gt_total_different},
                                 figure_type='float'),
            ],
            'level': 1,
            'unfoldable': False,
