...
class SummaryofOperationUSDExpenditures(models.AbstractModel):
    _name = "jt_finance.summaryof.operation.usd.expenditures"
    _inherit = "account.coa.report"
    _description = "Summary of Operation USD Expenditures"
    ...
    def _get_templates(self):
        templates = super(
            SummaryofOperationUSDExpenditures, self)._get_templates()
        templates[
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for journal in account_payment.mapped("journal_id"):
            ...
            if account_id:
                values= self.env['account.move.line'].search([('currency_id.name','=','USD'),('account_id', '=', account_id),('move_id.state', '=', 'posted')])
    ...
    def _get_report_name(self):
        return _("Summary of Operation-USD Expenditures")
    ...
    def get_xlsx(self, options, response=None):
        ...
        sheet.merge_range(y_offset, col, 5, col+2, header_title,super_col_style)
        y_offset += 6
        col=1
        currect_time_msg = "Fecha y hora de impresión: "
        currect_time_msg += datetime.today().strftime('%d/%m/%Y %H:%M')
        sheet.merge_range(y_offset, col, y_offset, col+2, currect_time_msg,currect_date_style)
        y_offset += 1
