...
class GeneralTotalPaymentAmounts(models.AbstractModel):
    _name = "jt_supplier_payment.general.total.payment.amounts"
    _inherit = "account.coa.report"
    _description = "General Total Payment Amounts"
    ...
    def _get_templates(self):
        templates = super(
            GeneralTotalPaymentAmounts, self)._get_templates()
        templates[
    ...
    def _get_columns_name(self, options):
        return [
            {'name': _('Banco de recepción de pago')},
...
#             options['date'].get('date_to'), '%Y-%m-%d').date()
    ...
    def _get_report_name(self):
        return _("General Total Payment Amounts")
...
#         y_offset += 1
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        if minimal_layout:
            ...
            header = self.env['ir.actions.report'].render_template("jt_supplier_payment.external_layout_general_total_payment_amounts", values=rcontext)
