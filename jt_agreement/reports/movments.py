...
class Movements(models.AbstractModel):
    _name = "jt_agreement.movements"
    ...
    def _get_report_name(self):
        return _("Movements Of Agreements")
    ...
    def _get_lines(self, options, line_id=None):
        ...
        operations = self.env['request.open.balance'].search([('bases_collaboration_id','!=',False),('state','=','confirmed'),('type_of_operation','=','increase'),('request_date', '>=', start), ('request_date', '<=', end)],order='request_date')
        total_amount = 0
        ...
        for base in base_ids:
            inc_opr = operations.filtered(lambda x:x.bases_collaboration_id.id == base.id)
            obs = ''
            if inc_opr:
                obs = inc_opr[0].observations
            ...
            lines.append({
                ...
                'columns': [{'name': ''}, 
                            {'name': base.name}, 
                            self._format({'name': base.interest_rate},figure_type='float'),
                            {'name': inc_opr and inc_opr[0].request_date}, 
                            {'name': ''},
                            {'name': obs},
                            ],
                'level': 3,
            ...
            total_base += base.interest_rate

        lines.append({
            'id': 'hierarchy_base_total',
            'name': '',
            'columns': [{'name': ''}, 
                        {'name': 'TOTAL'}, 
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        if minimal_layout:
            ...
            header = self.env['ir.actions.report'].render_template("jt_agreement.external_layout_agreements_movements", values=rcontext)
