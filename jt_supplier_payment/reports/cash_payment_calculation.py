...
class CashPaymentCalculation(models.AbstractModel):
    _name = "jt_supplier_payment.cash.payment.calculation"
    ...
    def _get_templates(self):
        templates = super(
            CashPaymentCalculation, self)._get_templates()
        templates[
    ...
    def _get_columns_name(self, options):
        return [
            {'name': _('Lugar de pago')},
            {'name': _('Descripción del lugar de pago')},
            {'name': _('Importe')},
            {'name': _('Total de depósitos')},
            {'name': _('Del')},
            {'name': _('Al')},
        ]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for place_id in payment_place_ids:
            ...
            total_rec += rec_len               
            lines.append({
                'id': 'hierarchy1_' + str(place_id.id),
                'name': place_id.name,
                'columns': [
    ...
    def _get_report_name(self):
        return _("Cash Payment Calculation")
...
#         y_offset += 1
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        if minimal_layout:
            ...
            header = self.env['ir.actions.report'].render_template("jt_supplier_payment.external_layout_cash_payment_calculation", values=rcontext)
