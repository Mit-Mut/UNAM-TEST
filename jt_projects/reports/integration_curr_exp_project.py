...
class InegrationOfExpAndCurrProject(models.AbstractModel):

    _name = "jt_projects.integration.current.expired.project"
    _inherit = "account.coa.report"
    _description = "Integration of current and expired projects"
    ...
    def _get_templates(self):
        templates = super(
            InegrationOfExpAndCurrProject, self)._get_templates()
        templates[
    ...
    def _get_columns_name(self, options):
        return [
            ...
            {'name': _('Entity')},
            {'name': _('Name')},
            {'name': _('No. of current projects')},
            {'name': _('Current projects')},
            {'name': _('No. Overdue projects')},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for dep in dep_ids:
            ...
            sub_dep_ids = project_records.filtered(
                lambda x: x.dependency_id.id == dep.id).mapped('subdependency_id')
            for sub in sub_dep_ids:
                ...
                total_project = 0
                open_project_ids = project_records.filtered(
                    lambda x: x.dependency_id.id == dep.id and x.subdependency_id.id == sub.id and x.status == 'open' and not x.check_project_due)
                open_project = len(open_project_ids)
                total_project += open_project

                ministring_amount = 0
                for p in open_project_ids:
                    ministring_amount += sum(
                        x.ministering_amount for x in p.project_ministrations_ids)

                verification_expense = self.env['expense.verification'].search(
                    [('project_id', 'in', open_project_ids.ids), ('status', '=', 'approve')])
                open_verification_amounts = sum(
                    x.amount_total for x in verification_expense)

                open_verification_amounts = ministring_amount - open_verification_amounts

                # over due project
                expired_projects_ids = project_records.filtered(
                    lambda x: x.dependency_id.id == dep.id and x.subdependency_id.id == sub.id and x.status == 'open' and x.check_project_due)
                expired_project = len(expired_projects_ids)

                over_due_ministring_amount = 0
                for p in expired_projects_ids:
                    over_due_ministring_amount += sum(
                        x.ministering_amount for x in p.project_ministrations_ids)

                overdue_verification_expense = self.env['expense.verification'].search(
                    [('project_id', 'in', expired_projects_ids.ids), ('status', '=', 'approve')])
                overdue_open_verification_amounts = sum(
...
#                 total_all_project += total_project
                ...
                lines.append({
                    ...
                    'columns': [{'name': entity},
                                ...
                                self._format(
                                    {'name': open_verification_amounts}, figure_type='float'),
...
#             lines.append(l)
        ...
        lines.append({
            ...
            'columns': [{'name': ''},
                        ...
                        self._format(
                            {'name': gt_total_open_verification_amount}, figure_type='float'),
                        {'name': gt_total_expired_project, 'class': 'number'},
                        self._format({'name': gt_total_over_dua},
                                     figure_type='float'),
                        self._format({'name': gt_total_per},
